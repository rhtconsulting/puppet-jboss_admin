...
define jboss_admin::resource::group_search_principal_to_group (
  ...
  if $ensure == present {
    ...
    if $skip_missing_groups != undef and $skip_missing_groups != undefined {
      validate_bool($skip_missing_groups)
    }
    ...
    $raw_options = {
      ...
      'skip-missing-groups'          => $skip_missing_groups,
    }
