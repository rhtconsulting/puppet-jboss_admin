# == Defines jboss_admin::blocking_bounded_queue_thread_pool
#
...
#
#
define jboss_admin::resource::blocking_bounded_queue_thread_pool (
  $server,
  ...
  if $ensure == present {
    ...
    if $allow_core_timeout != undef and !is_bool($allow_core_timeout) { 
      fail('The attribute allow_core_timeout is not a boolean') 
    }
