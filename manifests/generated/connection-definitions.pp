# == Defines jboss_admin::connection-definitions
#
...
#
#
define jboss_admin::connection-definitions (
  $server,
  ...
  if $ensure == present {
    ...
    if $xa_resource_timeout != undef && !is_integer($xa_resource_timeout) { 
      fail('The attribute xa_resource_timeout is not an integer') 
    }
    if $max_pool_size != undef && !is_integer($max_pool_size) { 
      fail('The attribute max_pool_size is not an integer') 
    }
    if $allocation_retry != undef && !is_integer($allocation_retry) { 
      fail('The attribute allocation_retry is not an integer') 
    }
    if $min_pool_size != undef && !is_integer($min_pool_size) { 
