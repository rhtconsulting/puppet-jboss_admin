# == Defines jboss_admin::blocking-bounded-queue-thread-pool
#
...
#
#
define jboss_admin::blocking-bounded-queue-thread-pool (
  $server,
  ...
  if $ensure == present {
    ...
    if $core_threads != undef && !is_integer($core_threads) { 
