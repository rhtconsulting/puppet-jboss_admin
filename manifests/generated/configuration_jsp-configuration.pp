...
#
#
define jboss_admin::configuration_jsp-configuration (
  $server,
  ...
  if $ensure == present {
    ...
    if $check_interval != undef && !is_integer($check_interval) { 
      fail('The attribute check_interval is not an integer') 
    }
    if $modification_test_interval != undef && !is_integer($modification_test_interval) { 
