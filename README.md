...
based on the documenation exported by the jboss-cli instead of hand made. The hope is that as
new resources are added this will simplify the process of adding manifests for 
them. The code backing generation is in `lib/tasks/generate.rb`, and the
...
manifests should go in `internal/custom_resource`. The generation
task will overlay these files over the autogenerated version.

### Required ###
* `JBOSS_CLI`: The full path to the jboss-cli.sh to run the generation against. Can be either a JBoss EAP or WildFly installation.
